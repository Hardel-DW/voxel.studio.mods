...
public final class StudioPermissionCommand {
    ...
    private static final SuggestionProvider<CommandSourceStack> SUGGEST_ROLES = (ctx, builder) -> SharedSuggestionProvider.suggest(new String[] { "admin", "contributor", "none" }, builder);
    ...
    private static int setRole(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        ctx.getSource().sendSuccess(() -> Component.translatable("studio:permission.role_set", target.getDisplayName(), roleName), true);
        return 1;
    }
    ...
    private static int showInfo(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        ctx.getSource().sendSuccess(() -> Component.translatable("studio:permission.info.header", target.getDisplayName()), false);
        ctx.getSource().sendSuccess(() -> Component.translatable("studio:permission.info.role", perms.role().getSerializedName()), false);
        return 1;
    }
