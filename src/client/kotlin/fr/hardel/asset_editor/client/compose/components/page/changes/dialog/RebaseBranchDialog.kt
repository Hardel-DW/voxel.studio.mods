...
@Composable
fun RebaseBranchDialog(
    visible: Boolean,
    snapshot: GitSnapshot,
    onDismiss: () -> Unit,
    onRebase: (name: String) -> Unit
) {
    if (!visible) return
    var query by remember { mutableStateOf("") }
    val current = snapshot.currentBranch

    val candidates = remember(snapshot.branches, current, query) {
    ...
    CommandPalette(
        visible = true,
        title = I18n.get("changes:menu.branch.rebase"),
        value = query,
        onValueChange = { query = it },
        placeholder = I18n.get("changes:dialog.rebase.placeholder"),
        leadingIcon = GIT_BRANCH_ICON,
        onDismiss = onDismiss
    ) {
        if (current == null) {
            CommandPaletteEmpty(I18n.get("changes:dialog.rebase.detached"))
            return@CommandPalette
        }
        val base = snapshot.branches.filter { it != current }
        if (base.isEmpty()) {
            CommandPaletteEmpty(I18n.get("changes:dialog.rebase.empty"))
            return@CommandPalette
        }
        CommandPaletteHint(
            I18n.get("changes:dialog.rebase.onto").replace("{branch}", current)
        )
        if (candidates.isEmpty()) {
            CommandPaletteEmpty(I18n.get("changes:dialog.rebase.empty_filter"))
            return@CommandPalette
        }
        LazyColumn(
            ...
            items(items = candidates, key = { it }) { branch ->
                CommandPaletteItem(
                    label = branch,
