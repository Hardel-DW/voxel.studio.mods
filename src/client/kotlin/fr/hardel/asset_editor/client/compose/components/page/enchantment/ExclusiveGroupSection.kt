...
fun ExclusiveGroupSection(
    ...
    val allEntries = context.allTypedEntries(Registries.ENCHANTMENT)
            ...
            items = ExclusiveSetGroup.ALL.map { group ->
                {
                    EnchantmentTags(
                        ...
                        imageId = group.image(),
                        values = resolveTagMembers(group.tagId, allEntries),
    ...
    Category(I18n.get("enchantment.exclusive:custom")) {
        val customTags = EnchantmentFlushAdapter.customExclusiveTags(allEntries)
                ...
                items = customTags.map { tagId ->
                    {
                        EnchantmentTags(
                            ...
                            values = resolveTagMembers(tagId, allEntries),
